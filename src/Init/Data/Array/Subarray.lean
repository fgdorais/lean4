...
def get (s : Subarray α) (i : Fin s.size) : α :=
  have : s.start + i.val < s.toArray.size := by
   apply Nat.lt_of_lt_of_le _ s.valid.right
   have := i.isLt
   simp [size] at this
   rw [Nat.add_comm]
   exact Nat.add_lt_of_lt_sub this
  s.toArray[s.start + i.val]
...
def popFront (s : Subarray α) : Subarray α :=
  if h : s.start < s.stop then
    { s with start := s.start + 1, valid := And.intro (Nat.le_of_lt_succ (Nat.add_lt_add_right h 1)) s.valid.right }
  else
    s
...
@[inline] unsafe def forInUnsafe {α : Type u} {β : Type v} {m : Type v → Type w} [Monad m] (s : Subarray α) (b : β) (f : α → β → m (ForInStep β)) : m β :=
  ...
  let rec @[specialize] loop (i : USize) (b : β) : m β := do
    if i < sz then
      let a := s.toArray.uget i lcProof
      match (← f a b) with
...
def toSubarray (as : Array α) (start : Nat := 0) (stop : Nat := as.size) : Subarray α :=
  if h₂ : stop ≤ as.size then
     if h₁ : start ≤ stop then
       {as with start := start, stop := stop, valid := And.intro h₁ h₂}
     else
       {as with start := stop, stop := stop, valid := And.intro (Nat.le_refl _) h₂}
  else
     if h₁ : start ≤ as.size then
       {as with start := start, stop := as.size, valid := And.intro h₁ (Nat.le_refl _)}
     else
       {as with start := as.size, stop := as.size, valid := And.intro (Nat.le_refl _) (Nat.le_refl _)}
...
def Subarray.extract (s : Subarray α) : Array α :=
  Array.ofSubarray s
